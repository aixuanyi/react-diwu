...
var CONNECT_STR = "mongodb://10.9.160.52:27017/five";

module.exports = {
  connect(res,callback){
...
//     console.log(result);
  ...
  insertProduct(db,collectionName,whereobj,insertData,callback){
  	db.collection(collectionName).update(whereobj,{$push:{'goods_list':insertData}},(err,result)=>{
  		if(err){
  			console.log(err)
  		}else{
//			console.log(result)
  ...
  insertMany(db, collectionName, insertData, callback ){
    db.collection(collectionName).insertMany(insertData, (err, result) => {
     if(err){
       console.log(err)
     }else{
//     console.log(result);
  ...
  findOne(db, collectionName, whereobj, callback){
    db.collection(collectionName).find(whereobj).toArray((err, result) => {
      if(err){
        console.log(err);
      }else{
        //result即为[{},{},{}]
...
//  moreValData   {$or:[{year:"1993"},{year:"1994"}]}
   ...
   updateData(db, collectionName, whereobj,updateobj, callback){
    db.collection(collectionName).update(whereobj,{$set:updateobj},(err,result)=>{
    	if(err){
    		console.log(err)
    	}else{
    		callback(result)
